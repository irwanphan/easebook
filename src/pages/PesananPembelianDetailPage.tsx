...
import { METODE_PEMBAYARAN_PEMBELIAN } from "@/data/pembelian";
import type {
  PesananPembelianDetail,
  PesananPembelianKonversiPayload,
...
export function PesananPembelianDetailPage() {
  ...
  const [detail, setDetail] = useState<PesananPembelianDetail | null>(null);
  ...
  const [konversiMetode, setKonversiMetode] = useState<string>("TUNAI");
  ...
  // Lazy-load daftar akun kas saat modal dibuka.
  useEffect(() => {
  ...
  async function handleKonversi() {
    ...
    if (!konversiMetode.trim()) {
      setKonversiError("Metode pembayaran wajib dipilih.");
      return;
    }
    const payload: PesananPembelianKonversiPayload = {
      tanggalFaktur: konversiTglFaktur.trim(),
      jatuhTempo: konversiJatuhTempo.trim(),
      metodePembayaran: konversiMetode.trim(),
    ...
    try {
      const nomorFaktur = await invoke<string>(
        "pesanan_pembelian_konversi_ke_faktur",
        { nomor: detail.nomor, payload },
      );
      // Stok bertambah saat konversi → refresh barang context.
      await refreshBarang();
      setKonversiOpen(false);
      navigate(`/pembelian/detail/${encodeURIComponent(nomorFaktur)}`, {
        replace: true,
      });
    } catch (err) {
  ...
  async function handleBatalkan() {
    ...
    setError(null);
    try {
      await invoke("pesanan_pembelian_batalkan", { nomor: detail.nomor });
  ...
  async function handleHapus() {
    ...
    setError(null);
    try {
      await invoke("pesanan_pembelian_delete", { nomor: detail.nomor });
      setHapusOpen(false);
      navigate("/pembelian/pesanan", { replace: true });
    } catch (e) {
      setError(tauriErrorMessage(e));
      setHapusBusy(false);
    }
      ...
      {detail && !loading ? (
        <>
            ...
            <Card className="border-emerald-200 bg-emerald-50/60">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-sm text-emerald-900">
                  ...
                  <p className="mt-0.5">
                    Stok telah bertambah saat pembuatan faktur. Klik tombol di
                    sebelah kanan untuk melihat faktur pembeliannya.
                  </p>
                </div>
                <Link
                  to={`/pembelian/detail/${encodeURIComponent(detail.fakturNomor)}`}
          ...
          <p className="text-xs text-zinc-500">
            Pesanan TIDAK menambah stok. Stok &amp; jurnal hanya berubah saat
            pesanan dikonversi menjadi faktur pembelian (barang diterima).
          </p>
        </>
      ) : null}
        ...
        >
          <div className="flex flex-col gap-4 text-sm">
            <p className="text-zinc-600">
              Faktur pembelian akan dibuat dengan pemasok, gudang, dan baris
              item yang sama dengan pesanan ini.
              <strong className="ml-1 text-zinc-800">
                Stok akan bertambah saat faktur tersimpan.
              </strong>
            </p>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <TokoSelect
                  id="po-konv-metode"
                  value={konversiMetode}
                  onChange={(e) => setKonversiMetode(e.target.value)}
                >
