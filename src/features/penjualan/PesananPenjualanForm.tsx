...
export type PesananPenjualanFormProps = {
  mode: "create" | "edit";
...
export function PesananPenjualanForm({
  mode,
  nomor,
  cancelHref,
  onSuccess,
}: PesananPenjualanFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (tanggalKirim.trim() && tanggalKirim < tanggalPesanan) {
      setError("Tanggal kirim tidak boleh sebelum tanggal pesanan.");
      return;
    }
    ...
    try {
      ...
      if (mode === "edit") {
        ...
        await invoke("pesanan_penjualan_update", {
          nomor: savedNomor,
          payload,
        });
      } else {
        savedNomor = await invoke<string>("pesanan_penjualan_insert", {
          payload,
        });
      }
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6">
      <div>
        ...
        <PageHeader
          title={mode === "edit" ? "Ubah pesanan penjualan" : "Pesanan penjualan baru"}
          description="Pesanan adalah komitmen jual dari pelanggan. Stok TIDAK berkurang di tahap ini — pengurangan stok terjadi saat pesanan dikonversi menjadi faktur penjualan."
        />
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <Card>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Pelanggan, gudang, tanggal pesanan & target kirim, plus salesman.
          </p>
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <label
                htmlFor="so-pelanggan"
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium text-zinc-700"
              >
                Salesman <span className="text-zinc-400">(opsional)</span>
              </label>
              <TokoInput
                id="so-salesman"
                type="text"
            ...
            <div>
              <label
                ...
                className="block text-sm font-medium text-zinc-700"
              >
                Target tanggal kirim <span className="text-zinc-400">(opsional)</span>
              </label>
              <TokoInput
                id="so-kirim"
                type="date"
        ...
        <Card className="overflow-hidden">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-500">
                Pilih barang/jasa, satuan, qty, dan harga. Stok cadangan TIDAK
                dipotong di sini — hanya tercatat sebagai pesanan.
              </p>
            </div>
            <Button
              type="button"
          ...
          <div className="mt-6 overflow-x-auto rounded-xl border border-zinc-100">
            <table className="w-full min-w-[1080px] text-left text-sm">
              ...
              <tbody className="divide-y divide-zinc-100">
                {lines.map((row) => {
                  const sub = penjualanLineSubtotal(
                    row.qty,
                    row.hargaSatuan,
                    row.diskon,
                  );
                      ...
                      <td className="px-3 py-2">
                        ...
                        {b?.tipe === "Barang" ? (
                          <p className="mt-1 text-xs text-zinc-500">
                            Stok tersedia: {b.stok ?? 0} {getSatuanStokBarang(b)}
                          </p>
                        ) : null}
                      </td>
                      <td className="px-3 py-2">
