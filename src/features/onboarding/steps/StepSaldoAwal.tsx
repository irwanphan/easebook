 ...
 *
 * Form lengkap untuk dua jenis saldo awal cukup kompleks (>500 baris
 * masing-masing, butuh password confirm, lock/unlock, dll) — meng-embed
 * mereka ke modal akan rentan bug & duplikasi logika. Sebagai gantinya:
 *
 *  - jelaskan konteksnya & tampilkan ringkasan jumlah entri yang ada,
 *  - tawarkan pintasan navigasi dalam-app (tanpa `target="_blank"`)
 *    ke halaman pengaturan asli,
 *  - `OnboardingGate` memberi izin path ini lewat `ONBOARDING_ESCAPE_HATCHES`
 *    sehingga admin tidak ditolak meski wizard belum selesai,
 *  - di halaman tujuan, `OnboardingResumeBanner` muncul sebagai jalan
 *    cepat balik ke wizard.
 *
    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      <div className="flex items-start gap-2 rounded-xl border border-sky-200 bg-sky-50/60 px-3 py-2.5 text-xs leading-relaxed text-sky-900">
        ...
        <span>
          ...
          mengisinya — Anda dapat kembali ke wizard ini lewat banner di atas halaman pengaturan.
          Atau klik Lanjut untuk melewati dan kembali kapan saja dari menu Keuangan / Barang.
        </span>
      </div>
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {rows.map((row, idx) => {
            const target = idx === 0 ? "/keuangan/kas-awal" : "/barang-jasa/atur-stok-awal";
            return (
              <li
                key={row.label}
