...
export function PesananPenjualanPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (q) {
        const hay =
          `${row.nomor} ${row.pelangganNama} ${row.salesman} ${row.status} ${row.fakturNomor ?? ""}`
            .toLowerCase();
    ...
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      <div>
        ...
        <PageHeader
          ...
          description="Komitmen jual yang belum dipotong stok. Saat siap dikirim, pesanan dapat dikonversi menjadi faktur penjualan."
          actions={
            <Button
              type="button"
      ...
      <Card className="overflow-hidden p-0">
        <ListFilterBar
          ...
          search={{
            value: query,
            onChange: setQuery,
            placeholder: "Cari no. pesanan, pelanggan, salesman, faktur…",
          }}
          selects={[
            {
              label: "Status",
                ...
                filteredRows.map((row) => (
                  <tr key={row.nomor} className="bg-white hover:bg-zinc-50/50">
                    ...
                    <td className="px-5 py-3 text-xs">
                      {row.fakturNomor ? (
                        <Link
                          to={`/penjualan/detail/${encodeURIComponent(row.fakturNomor)}`}
                    ...
                    <td className="px-5 py-3 text-right">
                      <Button
                        onClick={() =>
                          navigate(
                            `/penjualan/pesanan/detail/${encodeURIComponent(row.nomor)}`,
                          )
                        }
                        variant="outline"
                        className="px-2 py-1 text-xs"
