...
import type { PesananPembelianListRow } from "@/data/pesananPembelian";
...
export function PembelianPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (q) {
        const hay =
          `${row.nomor} ${row.pemasokNama} ${row.status}`.toLowerCase();
      ...
      <Card className="overflow-hidden p-0">
        <ListFilterBar
          ...
          search={{
            value: query,
            onChange: setQuery,
            placeholder: "Cari no. faktur, pemasok, atau status…",
          }}
          onReset={handleReset}
          canReset={!isDefault}
          errorMessage={
